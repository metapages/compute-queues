...
//   jobAge: number,
// ): number => {
//   // const isTesting = Deno.env.get("TESTING") === "true";
//   // if (isTesting && Deno.env.get(NAMESPACE_MULTIPLE_JOB_REPLACEMENT_THRESHOLD)) {
//   //   NAMESPACE_MULTIPLE_JOB_REPLACEMENT_THRESHOLD = ms(Deno.env.get(NAMESPACE_MULTIPLE_JOB_REPLACEMENT_THRESHOLD)) as number;
//   // }
...
// const replacementThreshold = parseInt(
//   Deno.env.get("JOB_REPLACEMENT_THRESHOLD_MS") || "60000",
// );
...
export class BaseDockerJobQueue {
      ...
      switch (payload.type) {
        case "job-states-minimal": {
          if (payload.origin === this.serverId) {
            console.log(
              `🌘 ...from merge ignoring job-states-minimal from ourselves:${payload.origin}`,
            );
            break;
          }
          ...
          for (let i = 0; i < jobStatesMinimal.length; i += 2) {
            ...
            if (
              ...
              console.log(
                `${this.addressShortString} 📡 found job ${
              ...
              (async () => {
                ...
                if (resolvedJob && resolvedJob !== this.state.jobs[jobId]) {
                  ...
                  console.log(
                    `${this.addressShortString} 📡 recieved job-states-minimal job different, broadcasting...`,
                    jobId,
                  );
        ...
        case "job-states": {
          ...
          if (payload.origin === this.serverId) {
            console.log(
              `🌘 ...from merge ignoring job-states from ourselves:${payload.origin}`,
            );
            break;
          }
          ...
          for (
            ...
            if (!this.state.jobs[jobId]) {
              console.log(
                `🌘 ...from merge adding jobId=${getJobColorizedString(jobId)}`,
              );
              ...
              if (
                ...
                this.state.jobs[jobId]?.state === job.state
              ) {
                // if the job is finished, and the state is finished, we don't need to resolve
        ...
        // https://github.com/metapages/compute-queues/issues/242
        case "remove-job": {
          // const jobId = (payload.value as BroadcastChannelRemoveJob).jobId;
          // const namespace =
          //   (payload.value as BroadcastChannelRemoveJob).namespace;
          // this.removeJobAndBroadcast(jobId, namespace);
          break;
        }
  ...
  async status(): Promise<object> {
    ...
    console.log(
      `${this.addressShortString} job states:`,
      jobStates,
    );
  ...
  async broadcastAndDeleteCachedJob(jobId: string): Promise<boolean> {
    ...
    if (!job) {
      console.log(
        `${this.addressShortString} 🗑️ NOT broadcastAndDeleteCachedJob no job ${
          getJobColorizedString(jobId)
        }`,
      );
      return false;
    ...
    if (!isJobCacheAllowedToBeDeleted(currentState)) {
      console.log(
        `${this.addressShortString} 🗑️ NOT broadcastAndDeleteCachedJob because it is in state ${job.state} [${
          getJobColorizedString(jobId)
        }]`,
      );
      return false;
  ...
  deleteCachedJob(jobId: string) {
    ...
    console.log(
      `${this.addressShortString} deletedCachedJob ${
        getJobColorizedString(
          jobId,
        )
      }`,
  ...
  /**
   * Does not automatically delete the job
   * from all queues/namespaces
   * @param jobId
   */
  // removeJobAndBroadcast(jobId: string, namespace?: string) {
  //   // https://github.com/metapages/compute-queues/issues/242
  //   return;
  //   // delete this.state.jobs[jobId];
  //   // namespace = namespace || DefaultNamespace;
  //   // const namespaces = await this.db.queueJobGetNamespaces(this.address, jobId);
  ...
  //   //   // this.channel.postMessage({
  //   //   //   type: "remove-job",
  //   //   //   value: { jobId, namespace: ns } as BroadcastChannelRemoveJob,
  //   //   // } as BroadcastChannelMessage);
  //   // }
  //   // console.log(
  ...
  protected async deleteJobFromDb(jobId: string) {
    ...
    console.log(
      `${this.addressShortString} deleted from db: [${
        getJobColorizedString(jobId)
      }]`,
    );
  }
          ...
          if (
            ...
            console.log(
              `🌘 got status from worker ${
                getWorkerColorizedString(status.id)
  ...
  async setup() {
    ...
    console.log(
      `${this.addressShortString} On startup got ${allPersistedJobInTheQueue.length} jobs from the db`,
    );
  ...
  dispose() {
    ...
    console.log(
      `${this.addressShortString} ➖ 🗑️ 🎾 UserDockerJobQueue `,
    );
  }
  ...
  public async stateChange(change: StateChange): Promise<void> {
    ...
    if (change && change.state) {
      console.log(
        `${getJobColorizedString(jobId)} stateChange ${
      ...
      switch (change.state) {
        ...
        case DockerJobState.Queued: {
          ...
          if (this.state.jobs[jobId]) {
            ...
            await updateState();
            // Check for source conflicts immediately after adding a new job
            this.checkForSourceConflicts();
          }
  ...
  async connectWorker(connection: { socket: WebSocket }, queue: string) {
    console.log(
      `${this.addressShortString} ➕ w 🔌 Connected a worker to queue, waiting for registration`,
    );
    ...
    connection.socket.addEventListener("close", () => {
      ...
      console.log(
        ...
        }] ➖ w 🔌 ⏹️ Removing ${workerIdColorized || "unknown"} after ${
    ...
    if (this.debug) {
      console.log(
        `⬅️ 📧 w sendJobStatesToWebsocket worker ${
          workerIdColorized || "unknown"
        }`,
      );
    }
  ...
  async connectClient(connection: { socket: WebSocket }) {
    console.log(
      `${this.addressShortString} ➕ c ⏯️ Connected a client`,
    );
    ...
    this.workers.myWorkers.forEach((worker) => {
      try {
        if (worker.connection.readyState === WebSocket.OPEN) {
          ...
          console.log(
            `🚨 Worker ${
              getWorkerColorizedString(worker.registration.id)
    ...
    if (failedWorkers.length > 0) {
      console.log(
        `🚨 Removing ${failedWorkers.length} failed workers: ${
          failedWorkers.map((id) => getWorkerColorizedString(id)).join(", ")
        }`,
      );
    ...
    for (
      ...
      if (job.state === DockerJobState.Running) {
        ...
        if (!workerIds.has(valueRunning.worker)) {
          console.log(
            `${this.addressShortString} 🚨  Requeuing job ${
              jobId.substring(
                0,
                6,
              )
  ...
  checkForSourceConflicts() {
    ...
    //   if (jobIds.length > 1) {
    //     console.log(
    //       `🚨 source conflict for ${namespace}: ${
    //         jobIds.map((id) => id.substring(0, 6)).join(", ")
    //       }`,
    //     );
    ...
    //       );
    //       // do not send a change because this job might be on a different queue
    //       this.stateChange({
