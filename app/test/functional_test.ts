  ...
  async () => {
    ...
    jobIdsSubmissionOrder.forEach((jobId, i) => {
      if (i === jobIdsSubmissionOrder.length - 1) {
        assertEquals(jobIdsFinishReason.get(jobId), DockerJobFinishedReason.Success);
      } else {
        assertEquals(jobIdsFinishReason.get(jobId), DockerJobFinishedReason.JobReplacedByClient);
      }
    });
