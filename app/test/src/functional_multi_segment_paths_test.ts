...
Deno.test(
  "Test job inputs and outputs with deeply nested file paths",
  async () => {
    // Create a temporary test file for input with deeply nested path
    let testInputContent = `Hello from deeply nested test input file!
This is a test file for validating job input access with very deep multi-segment paths.
Timestamp: ${Date.now()}`;
    while (testInputContent.length < ENV_VAR_DATA_ITEM_LENGTH_MAX) {
      testInputContent += testInputContent;
    }

    const testInputFile = `/tmp/test_input_deep_${Date.now()}.txt`;
    await Deno.writeTextFile(testInputFile, testInputContent);
    ...
    try {
      ...
      const definition = {
        image: "alpine:3.18.5",
        command:
          `sh -c 'mkdir -p /outputs/very/deep/nested/path/structure && echo "Processing deeply nested input file..." && cat /inputs/very/deep/nested/path/structure/test_input.txt > /outputs/very/deep/nested/path/structure/test_output.txt && echo "Job completed successfully"'`,
        inputs: {
          "very/deep/nested/path/structure/test_input.txt": inputDataref,
        },
      };
      ...
      // Test input file access with deeply nested path
      const inputResponse = await fetch(`${API_URL}/j/${jobId}/inputs/very/deep/nested/path/structure/test_input.txt`, {
        redirect: "manual", // Don't follow redirects automatically
      });
      ...
      // Test output file access with deeply nested path
      const outputResponse = await fetch(
        `${API_URL}/j/${jobId}/outputs/very/deep/nested/path/structure/test_output.txt`,
        {
          redirect: "manual", // Don't follow redirects automatically
        },
      );
      ...
      // Test queue-based endpoints with deeply nested paths as well
      const queueInputResponse = await fetch(
        `${API_URL}/q/${QUEUE_ID}/j/${jobId}/inputs/very/deep/nested/path/structure/test_input.txt`,
        {
          redirect: "manual",
        },
      );
      ...
      const queueOutputResponse = await fetch(
        `${API_URL}/q/${QUEUE_ID}/j/${jobId}/outputs/very/deep/nested/path/structure/test_output.txt`,
        {
          redirect: "manual",
        },
      );
      ...
      // Test error cases with deeply nested paths
      const nonExistentInputResponse = await originalFetch(
        `${API_URL}/j/${jobId}/inputs/very/deep/nested/path/structure/nonexistent.txt`,
      );
      assertEquals(nonExistentInputResponse.status, 404);
      nonExistentInputResponse.body?.cancel();

      const nonExistentOutputResponse = await originalFetch(
        `${API_URL}/j/${jobId}/outputs/very/deep/nested/path/structure/nonexistent.txt`,
      );
      assertEquals(nonExistentOutputResponse.status, 404);
      nonExistentOutputResponse.body?.cancel();
    } finally {
