...
export class DB {
  ...
  async queueJobGet(
    ...
    const entry = await kv.get<DataRef<DockerJobDefinitionRow>>([
      "queue",
      queue,
      id,
    ]);
  ...
  async queueGetAll(queue: string): Promise<DockerJobDefinitionRow[]> {
    const entries = kv.list<DataRef<DockerJobDefinitionRow>>({
      prefix: ["queue", queue],
    });
  ...
  async resultCacheGet(
    ...
    const cachedValueRefBlob = await kv.get<DataRef<DockerJobDefinitionRow>>([
      "cache",
      id,
    ]);
